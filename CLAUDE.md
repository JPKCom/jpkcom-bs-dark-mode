# JPKCom Bootstrap 5 Dark Mode Switch – Developer Reference

## Plugin Overview

Registers the `[jpkcom-bs-dark-mode]` shortcode, which outputs a Bootstrap 5 form-switch plus inline JavaScript that toggles `data-bs-theme` on `<html>` and persists the choice in `localStorage` (`bsTheme`), defaulting to dark.

- **Text Domain:** `jpkcom-bs-dark-mode` (no header declared, defaults to slug; only used by the shared updater)
...
```
Main file (jpkcom-bs-dark-mode.php)
├── declare(strict_types=1)
├── Plugin header
├── JPKCOM_BS_DARK_MODE_VERSION constant
├── init @ priority 5: boot JPKComGitPluginUpdater
├── jpkcom_bs_dark_mode_switch() : void  → echoes switch markup + toggle script
...
|-----------|----------|--------|
| `[jpkcom-bs-dark-mode]` | `jpkcom_bs_dark_mode_switch()` | Bootstrap form-switch + JS that sets `data-bs-theme` and stores `bsTheme` in `localStorage` |

The callback takes no attributes and echoes its markup (heredoc).
...
|----------|-------|---------|
| `JPKCOM_BS_DARK_MODE_VERSION` | matches the header `Version:` | Plugin version (sync with header/README/phpdoc.xml) |
...
```
jpkcom-bs-dark-mode/
├── jpkcom-bs-dark-mode.php       ← Main: header, constant, shortcode, updater bootstrap
├── includes/
│   └── class-plugin-updater.php  ← GitHub auto-updater (namespace: JPKComBsDarkModeGitUpdate)
...
- **Namespace:** `JPKComBsDarkModeGitUpdate\JPKComGitPluginUpdater`
- **Manifest URL:** `https://jpkcom.github.io/jpkcom-bs-dark-mode/plugin_jpkcom-bs-dark-mode.json`
...
- Static markup only (no user input processed)
...
1. Bump version in: header `Version:` + `Stable tag:`, `JPKCOM_BS_DARK_MODE_VERSION`, `README.md`, `phpdoc.xml`
